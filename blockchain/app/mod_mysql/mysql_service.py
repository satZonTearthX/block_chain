...
class Mysql_service(object):
    ...
    def register(self,user_name,password,email,role): #self,user,email,password,role
        sql='insert into all_users (user_name,password,email,role,account,credit)\
               values("%s","%s","%s","%s",0.0,100) ' % (user_name, password,email,role)
        try:
...
###################################改
    ...
    def modify_user_info(self,username,password=None,address=None,account=-1,email=None,host=None,port=-1):
        ...
        sql = '''select * from all_users where login_name="%s" ''' % username
        ...
        try:
            ...
            self.login_password = data["login_password"] if password is None else password
            self.address = data["address"] if address is None else address
            self.user_account = data["user_account"] if account ==-1 else account
            self.e_mail = data["e_mail"] if email is None else email
        ...
        sql2 = '''update all_users set login_password="%s",address="%s",user_account=%f,e_mail="%s",user_host="%s", user_port=%d\
                        where login_name="%s" ''' % (self.login_password, self.address, self.user_account,self.e_mail,self.user_host,self.user_port, username )
...
######################################增
    ...
    def add_tem_block(self):
        ...
        sql_create='''CREATE TABLE `block_tem` \
          ...
          CONSTRAINT `block_tem_ibfk_1` FOREIGN KEY (`buyer_name`) REFERENCES `all_users1` (`login_name`),\
          CONSTRAINT `block_tem_ibfk_2` FOREIGN KEY (`seller_name`) REFERENCES `all_users1` (`login_name`)\
